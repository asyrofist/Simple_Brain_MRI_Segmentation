...
if morphology1:
    ...
    erosion = np.zeros_like(erosion)
    erosion[labels == largest_label] = foreground_value
    st.image(erosion, caption='Gambar Erosion',use_column_width=True)
    ...
    st.image(opening, caption='Gambar Opening',use_column_width=True)
    ...
    dilasi = np.zeros_like(dilasi)
    dilasi[labels == largest_label] = foreground_value
    st.image(dilasi, caption='Gambar Dilation',use_column_width=True)
    ...
    segmented_image = np.zeros_like(dilasi)
    segmented_image[dilasi == foreground_value] = labels
    st.image(segmented_image, caption='Gambar Segmented',use_column_width=True)
